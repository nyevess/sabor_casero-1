    ...
    <img class="header__logo" src="../assets/img/svg/logo.svg" alt="" />
      ...
      <main class="main">
        ...
        ?></h2>
          ...
          <nav class="main__insertar__nav"><a href="#">
          ...
          ?>
            ...
            <?php
            //ahora hacemos un for each para que nos devuelva la información de los platos que  imprimirá la tarjeta con el plato img y descripcion tantas veces como platos haya
        ...
        <form class="ver__pedido" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
            
              <?php  
              include("script/carrito_junto.php");
              ?>
          <hr class="linea__division__footer"/>
               <label  class="label"  for="cliente"><b>Tu email:</b></label>
              <input id="cliente" type="hidden" name="cliente" value="<?php
              
       //nos conectamos y hacemos la consulta para obtener el email del cliente que esta haciendo el pedido. el input lo ocultamos para que no lo pueda modificar y mostramos el resultado en el siguiente echo    
       ...
       include("../pdo/conex.php");                
                ...
                $clientes->execute();
                $devuelve=$clientes->fetch(PDO::FETCH_ASSOC);
                   $ver=$devuelve['email'];
                  //  
                   echo $ver;                 
                ?>"><?php echo $ver;?><br>
              
              <label class="label" for="fecha">Cuándo quieres tu pedido?</label>
              <input type="date"  min="<?php echo date("Y-m-d",strtotime(date("Y-m-d")));?>"  name="fecha"><br>
              <label for="hora">Hora (desde 12:00 hasta 18:00):</label>
              <input type="time" value="12:00" min="12:00" max="18:00" step="900" name="hora"><br>
              <label for="recoger">Recoger o llevar:</label>
              <select class="input" name="recoger" id="recoger">
